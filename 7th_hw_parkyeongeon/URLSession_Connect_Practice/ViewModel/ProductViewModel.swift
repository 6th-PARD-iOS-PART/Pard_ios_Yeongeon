...
final class ProductViewModel: ObservableObject {
    ...
    func loadProductsColorCount(name: String) async {
        do {
            ...
            self.productColors = fetched
        } catch {
            print("GET Product Color&Count실패:", error)
        }
    }
    ...
    var filteredEachColorProducts: [ProductEachColor] {
        productEachColor
    ...
    var productNames: [String] {
        Array(
            Set(
                filteredProducts.compactMap { $0.name }
            )
        )
        .sorted()
    }
    ...
    var statusText: String {
        if selectedColor == "전체" {
            ...
            if filteredEachColorProducts.isEmpty {
    ...
    func colorTotals(for name: String) -> [(color: String, total: Int)] {
        ...
        return Dictionary(
            ...
            .map { color, items in
                (color: color,
                 total: items.reduce(0) { $0 + ($1.count)}
                )
            }
    ...
    func summary(for name: String) -> (price: Int, totalCount: Int, isSellable: Bool) {
        if selectedColor != "전체" {
            let items = productEachColor.filter { $0.name == name }
            guard !items.isEmpty else { return (0, 0, false) }

            let price = items.compactMap { $0.price }.min() ?? 0
        ...
        let price = items.compactMap(\.price).min() ?? 0
        let totalCount = items.reduce(0) { $0 + ($1.count) }
        let isSellable = items.contains { $0.sellable && $0.count > 0 }
