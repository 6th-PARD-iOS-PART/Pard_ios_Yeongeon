...
struct ContentView: View {
    ...
    private var colorSelectedSection: some View {
        ForEach(viewModel.productEachColor, id: \.self) { item in
            eachColorProductSection(for: item.name)
        }
    }
    ...
    @ViewBuilder
    private func eachColorProductSection(for name: String) -> some View {
        let summary = viewModel.summary(for: name)
        let priceText = formatPrice(summary.price)
        let countText = "\(summary.totalCount)"
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            eachColorDetailsSection(for: name)
                .transition(.move(edge: .bottom).combined(with: .opacity))
        }
    }
    ...
    @ViewBuilder
    private func eachColorDetailsSection(for name: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("색상별 재고 현황")
                .font(.headline)

            if let item = viewModel.productEachColor.first(where: { $0.color == viewModel.selectedColor }) {
                ColorDetailRow(
                    color: item.color,
                    total: item.count
                ) {
